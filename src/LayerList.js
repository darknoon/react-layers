...
export default class LayerList extends Component {
  ...
  helper = l => {
    ...
    const sub = l.sublayers ? <ul>{l.sublayers.map(sl => this.helper(sl))}</ul> : null;
    ...
    return (
      <li key={l.key}>
        <LayerItem
          ...
          onSelect={ e => {this.selectLayer(l.key); e.stopPropagation() } }
  ...
  render() {
    return (
      <div className='layer-list' onMouseDown={e => this.selectLayer(new Set())}>
